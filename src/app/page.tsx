...
const HomePage: React.FC = () => {
   ...
   */
  const [timeRemainingPercentage, setTimeRemainingPercentage] = useState<number>(0)

  /**
   * * Current countdown time in milliseconds
   * ? Updated every 100ms for real-time countdown display
   */
  const [countdownTimeLeft, setCountdownTimeLeft] = useState<number>(TARGET_DATE.getTime())
   ...
   */
  const [currentTimeDetails, setCurrentTimeDetails] = useState<ClockTimeDetails>({
    date: '',
    hours: 0,
    minutes: 0,
    seconds: 0,
  ...
  const fetchCurrentTaskData = useCallback(async (): Promise<void> => {
    try {
      ...
      if (response.data.taskDetails) {
        const transformedTasks: TodoItem[] = response.data.taskDetails.map(task => ({
          id: task._id,
          text: task.task,
          completed: task.done,
        }))
  ...
  useEffect(() => {
    ...
    const timeBreakdown: TimeBreakdown = {
      ...
      hours: Math.floor((countdownTimeLeft % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60)),
                  ...
                  {isLoading ? (
                    ...
                    <Card className="border-2 border-dashed bg-muted/30">
                      <CardContent className="py-12 text-center">
                        ...
                        <h4 className="mb-2 text-lg font-medium">No tasks yet</h4>
                        <p className="text-sm text-muted-foreground">
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="rounded-lg bg-muted/50 p-3">
                  ...
                  <Progress value={taskStats.completionRate} className="mt-2 h-1.5" />
                </div>
              </CardContent>
            </Card>
          </div>
